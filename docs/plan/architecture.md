...
- `db/` — `app_database.dart` (drift `AppDatabase`, **`schemaVersion => 8`**, `MigrationStrategy get migration`), `tables.dart` (incl. `enum CycleEventType {periodStart, pregnancyLoss, birth}`; `PainEntries` + `enum PainRegion` since v8, p7.5), `database_key_store.dart`, `app_database.g.dart`. Migrations v1–v8: v2–v6 additive `createTable`, v7 (p6.1) first ALTER (`source`/`external_id` cols, `to >= 7` guarded), v8 (p7.5) additive `pain_entries`.
- repositories — pattern: `<x>/<x>_repository.dart` (interface) + `<x>/drift_<x>_repository.dart` (impl). Covers `symptom/`, `settings/` (`app_settings` KV — typed-key seam for mode flags etc.), `meds/`, `mucus/`, `bbt/`, `flow/`, `period/`, `reminders/`, `repository/` (cycle events), `pain/` (`PainEntry` per-day endometriosis pain/flare log — rejects `SymptomSeverity.none`, note cap `kPainNoteMaxLength`, p7.5).
- `symptom/symptom_severity.dart` — pure `enum SymptomSeverity {none,mild,moderate,severe}` (`.rank` 0–3, `.label`, `.ordered`) — olf's first ordered symptom scale; stored by name in `pain_entries.intensity` (p7.5), reused by p7.6.
